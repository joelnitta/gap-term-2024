...
- 今のデータの幅がとても広い（行がたくさんある）

- 解析によっては、**長細い**形の方がいい時がある

- 幅広い形から細長い形に変えるには、`pivot_wider()`を使う
...
target_long <-
  blast_animals |>
    # まずは、種名とOTUだけに絞りましょう
    select(otu, contains("target_top")) |>
    # 行の名前を新しい「rank」という行に入れて、各セルのデータを「species」に入れましょう
    pivot_longer(names_to = "rank", values_to = "species", cols = -otu) |>
    # 最後にrankを数字にする
...
identity_long <-
  blast_animals |>
    select(otu, contains("identity_top")) |>
    pivot_longer(names_to = "rank", values_to = "identity", cols = -otu) |>
    mutate(rank = parse_number(rank))
...
- 最後に、この二つの細長いデータをくっ付ける（「join」）することによって、解析に使いやすい状態にする：

```{r}
left_join(target_long, identity_long)
```
